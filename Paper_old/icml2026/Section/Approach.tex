...
\emph{sequential} expert routing in \emph{non-stationary} time series under \emph{partial
feedback} \citep{neu2010online, dani2008stochastic}. All the notations used in the paper are indexed in Appendix \ref{app:notation}. 
...
\end{equation}
When expert \(I_t=k\in\mathcal{E}_t\) is queried (where \(I_t\) denotes the scalar index of the selected expert), the realized observation is \(e_t\coloneqq e_{t,I_t}\).
...
At each round, the router selects an expert index \(I_t\in\mathcal{E}_t\). Due to partial
...
\begin{enumerate}[topsep=0pt,itemsep=1pt,parsep=0pt]
    ...
    \item \textbf{partial feedback:} the router observes \(O_t=(I_t,\vhaty_{t,I_t},\vy_t)\) and can
...
\subsection{Generative Model: Factorized SLDS}
\label{sec:generative_model}

Section~\ref{sec:problem_formulation} highlights that partial feedback and non-stationarity make the
...
Section~\ref{sec:problem_formulation} as emissions of a \emph{factorized switching linear dynamical
system} \citep{bengio1994input, linderman2016recurrent, hu2024modeling}, which we call \textbf{L2D-SLDS}.
We begin with the core prediction equation that connects the latent state to the observed residuals,
then motivate each latent component in turn. For scalability under a
growing registry, our inference maintains per-expert marginals via a factorized filtering
approximation (Algorithm~\ref{alg:router_main}). The resulting linear-Gaussian structure yields
Kalman-style updates (Algorithm~\ref{alg:correct_reweight}) and closed-form information quantities
used in our routing rule. When the model parameters are not known a priori, they can be estimated
from a warmup prefix via Monte Carlo EM (Algorithm~\ref{alg:trainmodel_em}); the online routing
mechanism is self-contained and does not depend on a specific estimation procedure.

\subsubsection{Residual emission model}
\label{sec:emission_model}

We first present the emission model that maps latent states to predicted residuals.
Each expert \(k\) has a latent \emph{reliability} vector
\(\boldsymbol\alpha_{t,k}\coloneqq \mathbf{B}_k\mathbf{g}_t+\mathbf{u}_{t,k}\),
where \(\mathbf{g}_t\in\mathbb{R}^{d_g}\) is a shared global factor,
\(\mathbf{u}_{t,k}\in\mathbb{R}^{d_\alpha}\) is an expert-specific idiosyncratic state,
and \(\mathbf{B}_k\in\mathbb{R}^{d_\alpha\times d_g}\) is an expert-specific loading.
A discrete regime \(z_t\in\{1,\dots,M\}\) selects the active dynamical parameters.
Given regime \(z_t=m\) and context \(\mathbf{x}_t\), the signed residual follows a linear-Gaussian
emission:
...
Definition~\ref{def:l2d_slds_emission} is the core component of our L2D-SLDS: it
...
The emission model involves three latent components --- the shared factor \(\mathbf{g}_t\), the
idiosyncratic states \(\mathbf{u}_{t,k}\), and the discrete regime \(z_t\) --- each serving a distinct
role. We now motivate and define each one.

\subsubsection{Latent state hierarchy}
The hierarchy separates systemic shifts from expert-specific drifts.
A single queried residual updates the \emph{shared} factor
\(\mathbf{g}_t\), which immediately refines predictions for all experts. Expert-specific states
\(\mathbf{u}_{t,k}\) capture persistent idiosyncratic deviations that cannot be explained by global
conditions alone. The discrete regime \(z_t\) allows abrupt changes in the dynamics.

\textbf{Global factor dynamics.}
Under partial feedback, the only way to learn about \emph{unqueried} experts is to exploit structure
that couples them (see Proposition \ref{prop:cross_update}). The shared latent state
\(\mathbf{g}_t\in\mathbb{R}^{d_g}\) represents system-wide conditions (e.g., overall difficulty,
market volatility, sensor drift) that affect many experts simultaneously. Because \(\mathbf{g}_t\)
appears in every expert's reliability \eqref{eq:alpha_def}, updating \(\mathbf{g}_t\) from the single
observed residual \(e_t=e_{t,I_t}\) tightens the predictive beliefs for other experts \(k\neq I_t\),
...
The discrete regime \(z_t\in\{1,\dots,M\}\) selects the active dynamical parameters (e.g., ``bull'' vs.\ ``crisis'').
...
before observing the queried residual \(e_t\).
...
Together with the emission model (Definition~\ref{def:l2d_slds_emission}), the shared factor
\(\mathbf{g}_t\), idiosyncratic states \(\mathbf{u}_{t,k}\), and regime process \(z_t\) form a
complete generative model for expert residuals. Expert heterogeneity is expressed through (i) the
expert-specific state \(\mathbf{u}_{t,k}\) and (ii) expert-specific loadings \(\mathbf{B}_k\), which
determine how each expert responds to the shared factor.
...
\mathrm{Cov}\left(e_{t,j}^{\mathrm{pred}},e_{t,I_t}^{\mathrm{pred}}\mid \mathcal{G}_t\right)=\mathbf{0}.
    \end{aligned}
\end{equation*}
In particular, when the predictive cross-covariance is non-zero, observing \(e_t=e_{t,I_t}\) shifts
the conditional predictive mean of \(e_{t,j}^{\mathrm{pred}}\).
\end{restatable}
...
Under partial feedback, greedily selecting the expert with the lowest predicted cost can slow
adaptation by repeatedly querying a ``safe'' expert. We therefore use an IDS-inspired heuristic
\citep{russo2014learning} that trades off predicted cost against information gained about the latent
state \((z_t,\mathbf{g}_t)\). We emphasize that this objective is computed under our model-based
filtering approximation and is used as a pragmatic exploration criterion rather than as an
...
predictor. We define the predictive value gap $\Delta_t(k)
\coloneqq
...
\(\mathrm{IG}_t(k)=0\), IDS reduces to the myopic choice \(k_t^{\mathrm{pred}}\). We discuss this in details in Appendix \ref{app:info_gain}. 
...
\(\Delta_{\max}\) steps, where \(\Delta_{\max}\ge 1\) is a user-chosen staleness horizon $\mathcal{K}^{\mathrm{stale}}_t
\coloneqq
...
\prod_{k\in P_t} d\mathbf{u}_{t-1,k}.
    \end{aligned}
\end{equation*}
Then \(q^{\mathrm{pr}(P_t)}_{t-1\mid t-1}\) is exactly the marginal of \(q_{t-1\mid t-1}\) on the retained variables.
...
\textbf{Birth time update.}
At the predictive time (before observing any residual at round \(t\)), we propagate the
initialization prior \eqref{eq:birth_prior} through the idiosyncratic dynamics
\eqref{eq:idiosyncratic_dynamics}. In particular, for \(j\in\mathcal{E}^{\mathrm{init}}_t\) and \(z_t=m\),
\begin{equation}
\label{eq:birth_time_update}
\begin{aligned}
    \mathbf{u}_{t,j}\mid(\mathcal{F}_t,z_t=m)
\sim
\mathcal{N}\!\Big(\mathbf{A}^{(u)}_{m}\mu^{(m)}_{\mathrm{init},j},\ &\mathbf{A}^{(u)}_{m}\Sigma^{(m)}_{\mathrm{init},j}(\mathbf{A}^{(u)}_{m})^\top \\
& +\mathbf{Q}^{(u)}_{m}\Big).
\end{aligned}
\end{equation}
These moments are then used to form the one-step predictive residual distribution for the entering
expert via \eqref{eq:residual_emission}.
...
In the fixed-parameter online-routing regime, the per-step cost is
\(T_t^{\mathrm{online}}
=
\widetilde O\big(
C_\theta(\mathbf{x}_t) + M d_g^3 + M |\mathcal K_t| d_\alpha^3 + |\mathcal E_t|\, C_{\mathrm{score}}(M,d_g,d_\alpha,S)
\big)\)
with memory
...
\big)\),
where \(S\) is the Monte Carlo budget and \(C_\theta,C_{\mathrm{score}}\) are the transition-model and
per-expert scoring costs. Both scale with the maintained registry \(|\mathcal K_t| \le |\mathcal E_t|
+ \Delta_{\max}\), not the cumulative catalog.
