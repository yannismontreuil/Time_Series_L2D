...
\begin{itemize}
	    ...
	    \item Appendix~\ref{algo}: end-to-end router/filtering pseudocode and optional learning updates.
	    \item Appendix~\ref{app:cross_covariance}: exact (non-factorized) Kalman update cross-covariance for the queried update.
	    \item Appendix~\ref{app:info_gain}: information-gain derivations used for IDS-style exploration.
	    \item Appendix~\ref{app:proof_cross_update}--\ref{app:transfer}: proofs of propositions.
    \item Appendix~\ref{sec:experiments-details}: additional experiments.
\end{itemize}
...
We report the complete probabilistic graphical model of our L2D-SLDS with partial feedback and context-dependent
...
\end{tikzpicture}
%}
    \caption{L2D-SLDS with partial feedback and \emph{context-dependent} regime switching:
