...
class FactorizedSLDS(SLDSIMMRouter):
    ...
    Observation model (vector residual):
    ...
    def __init__(
        ...
        if self.N is None and isinstance(R, np.ndarray) and R.ndim == 4:
            if R.shape[0] == self.M:
                self.N = int(R.shape[1])
        if self.N is None and isinstance(R, np.ndarray) and R.ndim == 3:
            if R.shape[0] != self.M:
        ...
        self.R_mode = str(R_mode).lower()
        if self.R_mode not in ("auto", "scalar", "full"):
            raise ValueError("R_mode must be 'auto', 'scalar', or 'full'.")
        if self.R_mode == "auto":
            self.R_mode = "scalar"
            if np.ndim(self.R) >= 3:
                self.R_mode = "full"
            elif np.ndim(self.R) == 2:
                R_arr = np.asarray(self.R, dtype=float)
                if not (self.N is not None and R_arr.shape == (self.M, self.N)):
        ...
        self._eye_g = np.eye(self.d_g, dtype=float) if self.d_g > 0 else np.zeros((0, 0), dtype=float)
        self._eye_phi = np.eye(self.d_phi, dtype=float) if self.d_phi > 0 else np.zeros((0, 0), dtype=float)
        self._diag_flat_g = (
            np.arange(0, self.d_g * self.d_g, self.d_g + 1, dtype=int)
            if self.d_g > 0
            else np.zeros(0, dtype=int)
        )
        self._diag_flat_phi = (
            np.arange(0, self.d_phi * self.d_phi, self.d_phi + 1, dtype=int)
            if self.d_phi > 0
            else np.zeros(0, dtype=int)
        )
        self._A_g_diag: Optional[np.ndarray] = None
        self._A_u_diag: Optional[np.ndarray] = None
        self._A_g_outer: Optional[np.ndarray] = None
        self._A_u_outer: Optional[np.ndarray] = None
        if self.d_g > 0:
            A_g_diag = np.diagonal(self.A_g, axis1=1, axis2=2)
            if np.allclose(self.A_g, np.eye(self.d_g)[None, :, :] * A_g_diag[:, None, :]):
                self._A_g_diag = A_g_diag.copy()
                self._A_g_outer = self._A_g_diag[:, :, None] * self._A_g_diag[:, None, :]
        if self.d_phi > 0:
            A_u_diag = np.diagonal(self.A_u, axis1=1, axis2=2)
            if np.allclose(self.A_u, np.eye(self.d_phi)[None, :, :] * A_u_diag[:, None, :]):
                self._A_u_diag = A_u_diag.copy()
                self._A_u_outer = self._A_u_diag[:, :, None] * self._A_u_diag[:, None, :]
    ...
    def _normalize_R(self, R: float | np.ndarray):
        ...
        if R_arr.ndim == 2:
            if self.N is not None and R_arr.shape == (self.M, self.N):
                return R_arr
            if R_arr.shape[0] == R_arr.shape[1]:
                return R_arr
            raise ValueError("R must be (M, N) or square when 2D.")
        if R_arr.ndim == 3:
            if R_arr.shape[1] != R_arr.shape[2]:
                raise ValueError("R must have square trailing dims when 3D.")
            if R_arr.shape[0] == self.M:
                return R_arr
            if self.N is not None and R_arr.shape[0] == self.N:
                return R_arr
            return R_arr
        if R_arr.ndim == 4:
            if R_arr.shape[2] != R_arr.shape[3]:
                raise ValueError("R must have square trailing dims when 4D.")
            if R_arr.shape[0] != self.M:
                raise ValueError("R must have first dim M when 4D.")
            return R_arr
        raise ValueError("R must be scalar, (M,), (N,), (M, N), or a covariance array.")
    ...
    def _get_R(self, m: int, k: int) -> float:
        if np.ndim(self.R) == 0:
            return float(self.R)
        R_arr = np.asarray(self.R, dtype=float)
        if R_arr.ndim == 2 and self.N is not None and R_arr.shape == (self.M, self.N):
            return float(R_arr[m, k])
        raise ValueError("Scalar R requested but R is not scalar.")
    ...
    def _check_dy(self, d_y: int) -> int:
        if self.d_y is None:
            self.d_y = int(d_y)
        elif self.d_y != int(d_y):
            raise ValueError("Inconsistent residual dimension across calls.")
        return int(self.d_y)

    def _compute_phi(self, x: np.ndarray) -> np.ndarray:
        phi = np.asarray(self.feature_fn(x), dtype=float)
        if phi.ndim == 1:
            if phi.shape[0] != self.d_phi:
                raise ValueError("feature_fn returned wrong dimension for d_phi.")
            self._check_dy(1)
            return phi.reshape(self.d_phi, 1)
        if phi.ndim == 2:
            if phi.shape[0] != self.d_phi:
                raise ValueError("feature_fn returned wrong dimension for d_phi.")
            self._check_dy(phi.shape[1])
            return phi
        raise ValueError("feature_fn must return a vector or (d_phi, d_y) matrix.")

    def _as_residual_vector(self, residual: np.ndarray | float) -> np.ndarray:
        arr = np.asarray(residual, dtype=float)
        if arr.ndim == 0:
            vec = np.array([float(arr)], dtype=float)
        elif arr.ndim == 1:
            vec = arr.reshape(-1)
        else:
            raise ValueError("Residual must be scalar or 1D.")
        self._check_dy(vec.shape[0])
        return vec

    def _as_residual_matrix(self, residuals_full: np.ndarray) -> np.ndarray:
        arr = np.asarray(residuals_full, dtype=float)
        if arr.ndim == 1:
            self._check_dy(1)
            return arr.reshape(-1, 1)
        if arr.ndim == 2:
            self._check_dy(arr.shape[1])
            return arr
        raise ValueError("Full-feedback residuals must be 1D or 2D.")

    def _get_R_matrix(self, m: int, k: int, d_y: int) -> np.ndarray:
        if np.ndim(self.R) == 0:
            return np.eye(d_y, dtype=float) * float(self.R)
        R_arr = np.asarray(self.R, dtype=float)
        if R_arr.ndim == 2:
            if self.N is not None and R_arr.shape == (self.M, self.N):
                return np.eye(d_y, dtype=float) * float(R_arr[m, k])
            if R_arr.shape[0] != R_arr.shape[1]:
                raise ValueError("R matrix must be square.")
            if R_arr.shape[0] != d_y:
                raise ValueError("R dimension mismatch.")
            return R_arr
        if R_arr.ndim == 3:
            if R_arr.shape[1] != R_arr.shape[2]:
                raise ValueError("R covariance must have square trailing dims.")
            if R_arr.shape[0] == self.M:
                if R_arr.shape[1] != d_y:
                    raise ValueError("R dimension mismatch.")
                return R_arr[m]
            if R_arr.shape[0] == self.N:
                if R_arr.shape[1] != d_y:
                    raise ValueError("R dimension mismatch.")
                return R_arr[k]
            raise ValueError("R shape does not match M or N.")
        if R_arr.ndim == 4:
            if R_arr.shape[0] != self.M:
                raise ValueError("R shape mismatch for M.")
            if self.N is not None and R_arr.shape[1] != self.N:
                raise ValueError("R shape mismatch for N.")
            if R_arr.shape[2] != d_y or R_arr.shape[3] != d_y:
                raise ValueError("R dimension mismatch.")
            return R_arr[m, k]
        raise ValueError("Unsupported R shape.")

    def _get_R_scalars_for_expert(self, k: int) -> np.ndarray:
        if np.ndim(self.R) == 0:
            return np.full(self.M, float(self.R), dtype=float)
        R_arr = np.asarray(self.R, dtype=float)
        if R_arr.ndim == 2 and self.N is not None and R_arr.shape == (self.M, self.N):
            return np.asarray(R_arr[:, k], dtype=float).reshape(self.M)
        if R_arr.ndim == 4 and R_arr.shape[0] == self.M:
            return np.asarray(R_arr[:, k, 0, 0], dtype=float).reshape(self.M)
        if R_arr.ndim == 3 and R_arr.shape[0] == self.M and R_arr.shape[1:] == (1, 1):
            return np.asarray(R_arr[:, 0, 0], dtype=float).reshape(self.M)
        raise ValueError("Scalar-R view is only available for scalar-output observations.")

    def _ensure_full_R(self, d_y: int) -> None:
        if np.ndim(self.R) == 4:
            return
        if self.N is None:
            raise ValueError("num_experts required to expand full R.")
        M = int(self.M)
        N = int(self.N)
        if np.ndim(self.R) == 0:
            base = float(self.R)
            R_full = np.zeros((M, N, d_y, d_y), dtype=float)
            for m in range(M):
                for k in range(N):
                    R_full[m, k] = np.eye(d_y, dtype=float) * base
            self.R = R_full
            return
        R_arr = np.asarray(self.R, dtype=float)
        if R_arr.ndim == 2:
            if R_arr.shape == (M, N):
                ...
                for m in range(M):
                    for k in range(N):
                        R_full[m, k] = np.eye(d_y, dtype=float) * float(R_arr[m, k])
                self.R = R_full
                return
            if R_arr.shape[0] == R_arr.shape[1] == d_y:
                R_full = np.zeros((M, N, d_y, d_y), dtype=float)
                for m in range(M):
                    for k in range(N):
                        R_full[m, k] = R_arr
                ...
                return
        if R_arr.ndim == 3:
            if R_arr.shape[1] != R_arr.shape[2] or R_arr.shape[1] != d_y:
                raise ValueError("R dimension mismatch for full expansion.")
            if R_arr.shape[0] == M:
                R_full = np.zeros((M, N, d_y, d_y), dtype=float)
                for m in range(M):
                    for k in range(N):
            ...
            if R_arr.shape[0] == N:
                ...
                self.R = R_full
                return
        raise ValueError("Unable to expand R to full covariance shape.")

    def _logdet_psd(self, cov: np.ndarray) -> float:
        ...
        for i in range(5):
            ...
            try:
                L = np.linalg.cholesky(cov + jitter * np.eye(d, dtype=float))
    ...
    def _loss_from_residual(self, residual: np.ndarray) -> float:
        residual = np.asarray(residual, dtype=float)
        if self.loss_fn is None:
            return float(np.sum(residual * residual))
        val = self.loss_fn(residual)
        val_arr = np.asarray(val, dtype=float)
        if val_arr.ndim == 0:
            return float(val_arr)
        return float(np.sum(val_arr))
    ...
    def _sanitize_vector(self, vec: np.ndarray, fallback: np.ndarray) -> np.ndarray:
        ...
        if max_abs <= self.state_clip:
            return vec
        out = vec.copy()
        np.clip(out, -self.state_clip, self.state_clip, out=out)
        return out
    ...
    def _stabilize_cov_fast(
        self,
        cov: np.ndarray,
        fallback: np.ndarray,
        diag_flat_idx: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        """Fast covariance stabilization for hot filtering paths.

        In the interaction/prediction step the covariance is built from convex
        combinations of PSD matrices plus process noise, so it is PSD by
        construction up to numerical drift. In that setting we avoid repeated
        Cholesky/eigendecomposition and only enforce finiteness, symmetry, entry
        clipping, and a positive diagonal floor. If the matrix contains invalid
        values we still fall back to the slower robust sanitizer.
        """
        cov = np.asarray(cov, dtype=float)
        if not np.isfinite(cov).all():
            return self._sanitize_cov(cov, fallback)
        if cov.size == 0:
            return cov.copy()
        if cov.shape == (1, 1):
            val = float(cov[0, 0])
            if not np.isfinite(val):
                return np.asarray(fallback, dtype=float).copy()
            val = min(max(val, self.eps), self.cov_clip)
            return np.array([[val]], dtype=float)
        out = 0.5 * (cov + cov.T)
        if self.cov_clip < 1.0e10:
            np.clip(out, -self.cov_clip, self.cov_clip, out=out)
        flat = out.reshape(-1)
        if diag_flat_idx is None:
            step = out.shape[0] + 1
            diag_flat_idx = np.arange(0, flat.size, step, dtype=int)
        diag_vals = flat[diag_flat_idx]
        diag_vals = np.minimum(np.maximum(diag_vals, self.eps), self.cov_clip)
        if self.eps > 0.0:
            diag_vals = diag_vals + self.eps
        flat[diag_flat_idx] = diag_vals
        return out
    ...
    def _log_debug_diagnostics(
        ...
        if cfg.get("log_variances", True) and stats is not None:
            ...
            for k, stat in stats.items():
                cov_modes = stat.get("cov", None)
                if cov_modes is not None:
                    cov_arr = np.asarray(cov_modes, dtype=float)
                    if cov_arr.ndim == 3 and cov_arr.shape[1:] == (1, 1):
                        var_modes = cov_arr.reshape(-1)
                    elif cov_arr.ndim == 3:
                        var_modes = np.array(
                            [float(np.trace(cov_arr[m])) for m in range(cov_arr.shape[0])],
                            dtype=float,
                        )
                    else:
                        var_modes = cov_arr.reshape(-1)
    ...
    def _record_em_observation(
        ...
        record = {
            ...
            "action": int(action),
            "residual": self._as_residual_vector(residual).copy(),
            "residual_full": None
    ...
    def _interaction_and_time_update(
        ...
        if self.d_g == 0:
            ...
            mixed_mu_g = mu_ij.T @ mu_g_prev
            diff_g = np.clip(
                mu_g_prev[:, None, :] - mixed_mu_g[None, :, :],
                -self.state_clip,
                self.state_clip,
            )
            mixed_Sigma_g = np.einsum("ij,iab->jab", mu_ij, Sigma_g_prev)
            mixed_Sigma_g += np.einsum("ij,ija,ijb->jab", mu_ij, diff_g, diff_g)
            for j in range(self.M):
                if not np.isfinite(mixed_mu_g[j]).all():
                    mixed_mu_g[j] = self.g_mean0[j].copy()
                if not np.isfinite(mixed_Sigma_g[j]).all():
                    mixed_Sigma_g[j] = self.g_cov0[j].copy()
        ...
        for k in self.registry:
            ...
            mixed_mu = mu_ij.T @ mu_modes
            diff_u = np.clip(
                mu_modes[:, None, :] - mixed_mu[None, :, :],
                -self.state_clip,
                self.state_clip,
            )
            mixed_Sigma = np.einsum("ij,iab->jab", mu_ij, Sigma_modes)
            mixed_Sigma += np.einsum("ij,ija,ijb->jab", mu_ij, diff_u, diff_u)
            for j in range(self.M):
                if not np.isfinite(mixed_mu[j]).all():
                    mixed_mu[j] = self.u_mean0[j].copy()
                if not np.isfinite(mixed_Sigma[j]).all():
                    mixed_Sigma[j] = self.u_cov0[j].copy()
            mixed_mu_u[k] = mixed_mu
            mixed_Sigma_u[k] = mixed_Sigma
        ...
        if self.d_g > 0:
            if self._A_g_diag is not None and self._A_g_outer is not None:
                mu_g_pred = mixed_mu_g * self._A_g_diag
                Sigma_g_pred = mixed_Sigma_g * self._A_g_outer + self.Q_g
            else:
                mu_g_pred = np.einsum("mij,mj->mi", self.A_g, mixed_mu_g)
                tmp_g = np.matmul(self.A_g, mixed_Sigma_g)
                Sigma_g_pred = np.matmul(tmp_g, np.transpose(self.A_g, (0, 2, 1))) + self.Q_g
            for m in range(self.M):
                Sigma_g_pred[m] = self._stabilize_cov_fast(
                    Sigma_g_pred[m],
                    self.g_cov0[m],
                    self._diag_flat_g,
                )
        ...
        for k in self.registry:
            if self._A_u_diag is not None and self._A_u_outer is not None:
                mu_u_pred[k] = mixed_mu_u[k] * self._A_u_diag
                Sigma_u_pred[k] = mixed_Sigma_u[k] * self._A_u_outer + self.Q_u
            else:
                mu_u_pred[k] = np.einsum("mij,mj->mi", self.A_u, mixed_mu_u[k])
                tmp_u = np.matmul(self.A_u, mixed_Sigma_u[k])
                Sigma_u_pred[k] = (
                    np.matmul(tmp_u, np.transpose(self.A_u, (0, 2, 1))) + self.Q_u
                )
            for m in range(self.M):
                Sigma_u_pred[k][m] = self._stabilize_cov_fast(
                    Sigma_u_pred[k][m],
                    self.u_cov0[m],
                    self._diag_flat_phi,
                )
    ...
    def _compute_predictive_stats(
        ...
        stats: Dict[int, dict] = {}
        need_sampling_stats = self.exploration == "sampling"

        phi_T = phi.T
        d_y = int(phi_T.shape[0])
        if d_y == 1 and not need_sampling_stats:
            phi_vec = phi[:, 0]
            for k in available_experts:
                ...
                cov_modes = np.zeros(self.M, dtype=float)
                loss_modes = np.zeros(self.M, dtype=float)
                ig_modes = np.zeros(self.M, dtype=float)

                b_modes = mu_u_pred[k] @ phi_vec
                noise_modes = np.einsum(
                    "i,mij,j->m",
                    phi_vec,
                    Sigma_u_pred[k],
                    phi_vec,
                ...
                if self.R_mode == "scalar":
                    if np.ndim(self.R) == 2 and self.N is not None:
                        noise_modes = noise_modes + np.asarray(self.R, dtype=float)[:, k]
                    else:
                        noise_modes = noise_modes + float(self.R)
                else:
                    noise_modes = np.array(
                        [
                            float(
                                phi_T @ Sigma_u_pred[k][m] @ phi
                                + self._get_R_matrix(m, k, d_y)
                            )
                            for m in range(self.M)
                        ],
                        dtype=float,
                    )
                noise_modes = np.maximum(noise_modes, self.eps)

                if self.d_g > 0:
                    Bk = self._ensure_B(k)
                    h_g = Bk.T @ phi_vec
                    mean_modes = mu_g_pred @ h_g + b_modes
                    signal_modes = np.einsum(
                        "i,mij,j->m",
                        h_g,
                        Sigma_g_pred,
                        h_g,
                    )
                    signal_modes = np.maximum(signal_modes, 0.0)
                else:
                    mean_modes = b_modes
                    signal_modes = np.zeros(self.M, dtype=float)

                cov_modes = signal_modes + noise_modes
                if self.loss_fn is None:
                    loss_modes = cov_modes + mean_modes * mean_modes
                else:
                    for m in range(self.M):
                        loss_modes[m] = self._expected_loss_gaussian(
                            np.array([mean_modes[m]], dtype=float),
                            cov_modes[m],
                        )
                ig_modes = 0.5 * np.log1p(signal_modes / noise_modes)
        ...
        for k in available_experts:
            ...
            if self.d_g > 0:
                ...
                H_g = np.zeros((d_y, 0), dtype=float)
            mean_modes = np.zeros((self.M, d_y), dtype=float)
            cov_modes = np.zeros((self.M, d_y, d_y), dtype=float)
            loss_modes = np.zeros(self.M, dtype=float)
            signal_modes = (
                np.zeros((self.M, d_y, d_y), dtype=float)
                if need_sampling_stats
                else None
            )
            noise_modes = (
                np.zeros((self.M, d_y, d_y), dtype=float)
            ...
            b_modes = (
                np.zeros((self.M, d_y), dtype=float)
                if need_sampling_stats
                else None
            )
            ig_modes = np.zeros(self.M, dtype=float)
            for m in range(self.M):
                ...
                b_m = phi_T @ mu_u_m
                noise_cov = phi_T @ Sigma_u_m @ phi + self._get_R_matrix(m, k, d_y)
                noise_cov = 0.5 * (noise_cov + noise_cov.T)
                if self.d_g > 0:
                    mu_g_m = mu_g_pred[m]
                    Sigma_g_m = Sigma_g_pred[m]
                    mean_m = H_g @ mu_g_m + b_m
                    signal_cov = H_g @ Sigma_g_m @ H_g.T
                    signal_cov = 0.5 * (signal_cov + signal_cov.T)
                else:
                    mean_m = b_m
                    signal_cov = np.zeros((d_y, d_y), dtype=float)
                cov_m = signal_cov + noise_cov
                mean_modes[m] = mean_m
                cov_modes[m] = cov_m
                loss_modes[m] = self._expected_loss_gaussian(mean_m, cov_m)
                if need_sampling_stats:
                    signal_modes[m] = signal_cov
                    noise_modes[m] = noise_cov
                    b_modes[m] = b_m
                if d_y == 1:
                    denom = max(float(noise_cov[0, 0]), self.eps)
                    numer = max(float(signal_cov[0, 0]), 0.0)
                    ig_modes[m] = 0.5 * np.log1p(numer / denom)
                else:
                    S = noise_cov
                    try:
                        S_inv = np.linalg.solve(S, signal_cov)
                        ig_modes[m] = 0.5 * max(
                            self._logdet_psd(np.eye(d_y) + S_inv), 0.0
                        )
                    except np.linalg.LinAlgError:
                        ig_modes[m] = 0.0
    ...
    def _expected_loss_gaussian(self, mean: np.ndarray, cov: np.ndarray) -> float:
        mean_arr = np.asarray(mean, dtype=float).reshape(-1)
        if mean_arr.size == 0:
            return 0.0
        cov_arr = np.asarray(cov, dtype=float)
        if cov_arr.ndim == 0:
            var = max(float(cov_arr), self.eps)
            if self.loss_fn is None:
                return var + float(mean_arr[0]) ** 2
            if self.loss_quad_points <= 0 or var <= self.eps:
                return float(self._apply_loss_fn(np.array([mean_arr[0]], dtype=float))[0])
            xs, ws = np.polynomial.hermite.hermgauss(self.loss_quad_points)
            samples = float(mean_arr[0]) + np.sqrt(2.0 * var) * xs
            vals = self._apply_loss_fn(samples)
            return float(np.sum(ws * vals) / np.sqrt(np.pi))

        if cov_arr.shape != (mean_arr.shape[0], mean_arr.shape[0]):
            raise ValueError("Covariance dimension mismatch in expected loss.")
        cov_arr = 0.5 * (cov_arr + cov_arr.T)
        if self.loss_fn is None:
            return float(np.trace(cov_arr) + mean_arr @ mean_arr)
    ...
    def _estimate_iz(
        ...
        if M <= 1 or n_samples <= 0:
            return 0.0
        mean_arr = np.asarray(mean_modes, dtype=float)
        cov_arr = np.asarray(cov_modes, dtype=float)
        if log_w is None:
            log_w = np.log(np.maximum(w_pred, self.eps))
        ig = 0.0
        if cov_arr.ndim == 1 and mean_arr.ndim == 1 and cov_arr.shape[0] == M:
            mean_vec = mean_arr
            var_vec = np.maximum(cov_arr, self.eps)
            log_norm = -0.5 * np.log(2.0 * np.pi * var_vec)
            for m in range(M):
                weight = float(w_pred[m])
                if weight <= 0.0:
                    continue
                samples = self._rng.normal(
                    loc=float(mean_vec[m]),
                    scale=float(np.sqrt(var_vec[m])),
                    size=int(n_samples),
                )
                centered = samples[None, :] - mean_vec[:, None]
                log_p_all = log_norm[:, None] - 0.5 * (centered * centered) / var_vec[:, None]
                log_p_m = log_p_all[m]
                mix_terms = log_w[:, None] + log_p_all
                vmax = np.max(mix_terms, axis=0)
                log_mix = vmax + np.log(np.sum(np.exp(mix_terms - vmax[None, :]), axis=0))
                ig += weight * float(np.mean(log_p_m - log_mix))
            return max(float(ig), 0.0)
        if cov_arr.ndim == 3 and cov_arr.shape[1:] == (1, 1):
            mean_vec = mean_arr.reshape(-1)
            var_vec = cov_arr.reshape(-1)
            var_vec = np.maximum(var_vec, self.eps)
            log_norm = -0.5 * np.log(2.0 * np.pi * var_vec)
            for m in range(M):
                weight = float(w_pred[m])
                if weight <= 0.0:
        ...
        for m in range(M):
            ...
            for j in range(M):
                log_p_all[j] = self._gaussian_logpdf_mvn_batch(
                    samples, mean_arr[j], cov_arr[j]
                )
    ...
    def _compute_iz_scores(
        ...
        for k, stat in stats.items():
            iz_scores[int(k)] = self._estimate_iz(
                stat["mean"], stat["cov"], w_pred, n_samples, log_w=log_w
            )
        return iz_scores
    ...
    def _sample_predictive_costs_from_stats(
        ...
        H = np.stack([stats[k]["H_g"] for k in avail], axis=0)
        b_modes = np.stack([stats[k]["b"] for k in avail], axis=0)
        noise_modes = np.stack([stats[k]["noise"] for k in avail], axis=0)
        ...
        for n in range(int(n_samples)):
            ...
            if deterministic:
                cost = np.array(
                    [
                        self._expected_loss_gaussian(mean[i], noise_modes[i, z])
    ...
    def _gaussian_logpdf_mvn_batch(
        self,
        x: np.ndarray,
        mean: np.ndarray,
        cov: np.ndarray,
        ...
        mean_arr = np.asarray(mean, dtype=float).reshape(-1)
        if mean_arr.shape[0] != d:
            raise ValueError("Mean dimension mismatch in batched multivariate logpdf.")
        cov_arr = np.asarray(cov, dtype=float)
        if d == 1:
            if cov_arr.shape == ():
                var = float(cov_arr)
                ...
                raise ValueError("Covariance dimension mismatch in batched multivariate logpdf.")
            var = max(float(var), self.eps)
            diff = x_arr[:, 0] - float(mean_arr[0])
            return -0.5 * (np.log(2.0 * np.pi * var) + (diff * diff) / var)
        if cov_arr.shape != (d, d):
            raise ValueError("Covariance dimension mismatch in batched multivariate logpdf.")
        cov_arr = 0.5 * (cov_arr + cov_arr.T)
        diff = x_arr - mean_arr[None, :]
    ...
    def _update_from_predicted(
        self,
        selected_expert: int,
        observed_value: float | np.ndarray,
        phi: np.ndarray,
        w_pred: np.ndarray,
        mu_g_pred: np.ndarray,
        Sigma_g_pred: np.ndarray,
        ...
        if d_y == 1:
            ...
            if self.d_g > 0:
                H_g = Bk.T @ phi_vec
                cross_g = np.einsum("mij,j->mi", Sigma_g_pred, H_g)
                signal = np.einsum("i,mi->m", H_g, cross_g)
                pred_g = mu_g_pred @ H_g
            else:
                H_g = np.zeros(0, dtype=float)
                cross_g = np.zeros((self.M, 0), dtype=float)
                signal = np.zeros(self.M, dtype=float)
                pred_g = np.zeros(self.M, dtype=float)
            cross_u = np.einsum("mij,j->mi", Sigma_u_pred[k], H_u)
            pred_u = mu_u_pred[k] @ H_u
            if self.R_mode == "scalar":
                if np.ndim(self.R) == 2 and self.N is not None:
                    R_modes = np.asarray(self.R, dtype=float)[:, k]
                else:
                    R_modes = np.full(self.M, float(self.R), dtype=float)
            else:
                R_modes = np.array(
                    [float(self._get_R_matrix(m, k, 1)[0, 0]) for m in range(self.M)],
                    dtype=float,
                )
            S = signal + np.einsum("i,mi->m", H_u, cross_u) + R_modes
            if not np.all(np.isfinite(S)):
                return self._update_from_predicted_generic(
                    selected_expert,
                    observed_value,
                    phi,
                    w_pred,
            ...
            pred_mean = pred_g + pred_u
            innovation = obs - pred_mean
            gain_scale = innovation / S
            inv_S = 1.0 / S
            if self.d_g > 0:
                mu_g_post = mu_g_pred + cross_g * gain_scale[:, None]
                Sigma_g_post = Sigma_g_pred - np.einsum(
                    "mi,mj,m->mij",
                    cross_g,
                    cross_g,
                    inv_S,
                ...
                for m in range(self.M):
                    Sigma_g_post[m] = self._stabilize_cov_fast(
                        Sigma_g_post[m],
                        self.g_cov0[m],
                        self._diag_flat_g,
                    )
            mu_u_post[k] = mu_u_pred[k] + cross_u * gain_scale[:, None]
            Sigma_u_post[k] = Sigma_u_pred[k] - np.einsum(
                "mi,mj,m->mij",
                cross_u,
                cross_u,
                inv_S,
            ...
            for m in range(self.M):
                Sigma_u_post[k][m] = self._stabilize_cov_fast(
                    Sigma_u_post[k][m],
                    self.u_cov0[m],
                    self._diag_flat_phi,
                )
            log_like = self._gaussian_logpdf_vec(obs, pred_mean, S)
        ...
        for m in range(self.M):
            ...
            H_g = phi_T @ Bk
            H_u = phi_T
            pred_mean = H_g @ mu_g + H_u @ mu_u
            R_mat = self._get_R_matrix(m, k, d_y)
            cross_g = Sigma_g @ H_g.T
            cross_u = Sigma_u @ H_u.T
            S = H_g @ cross_g + H_u @ cross_u + R_mat
            S = 0.5 * (S + S.T)

            try:
                L = np.linalg.cholesky(S + self.eps * np.eye(d_y, dtype=float))
                tmp_g = np.linalg.solve(L, cross_g.T)
                K_g = np.linalg.solve(L.T, tmp_g).T
                tmp_u = np.linalg.solve(L, cross_u.T)
                K_u = np.linalg.solve(L.T, tmp_u).T
            except np.linalg.LinAlgError:
                S_reg = S + self.eps * np.eye(d_y, dtype=float)
                K_g = np.linalg.solve(S_reg, cross_g.T).T
                K_u = np.linalg.solve(S_reg, cross_u.T).T
            innovation = obs_vec - pred_mean
            mu_g_post[m] = mu_g + K_g @ innovation
            mu_u_post[k][m] = mu_u + K_u @ innovation
            Sigma_g_post[m] = self._stabilize_cov_fast(
                Sigma_g - K_g @ cross_g.T,
                self.g_cov0[m],
                self._diag_flat_g,
            )
            Sigma_u_post[k][m] = self._stabilize_cov_fast(
                Sigma_u - K_u @ cross_u.T,
                self.u_cov0[m],
                self._diag_flat_phi,
            )

            log_like[m] = self._gaussian_logpdf_mvn(obs_vec, pred_mean, S)
    ...
    def _update_from_predicted_generic(
        self,
        selected_expert: int,
        observed_value: float | np.ndarray,
        phi: np.ndarray,
    ...
    def _update_from_predicted_full(
        ...
        obs_arr = self._as_residual_matrix(observed_residuals)
        d_y = int(obs_arr.shape[1])
        ...
        H = np.zeros((n_avail * d_y, dim), dtype=float)
        phi_T = phi.T
        for idx, k in enumerate(available):
            row_start = idx * d_y
            row_end = row_start + d_y
            if self.d_g > 0:
                Bk = self._ensure_B(k)
                H[row_start:row_end, : self.d_g] = (phi_T @ Bk)
            if self.d_phi > 0:
                start = self.d_g + idx * self.d_phi
                H[row_start:row_end, start : start + self.d_phi] = phi_T
        ...
        for m in range(self.M):
            ...
            R_blocks = [self._get_R_matrix(m, k, d_y) for k in available]
            R_diag = np.zeros((n_avail * d_y, n_avail * d_y), dtype=float)
            for idx, Rk in enumerate(R_blocks):
                start = idx * d_y
                R_diag[start : start + d_y, start : start + d_y] = Rk
            pred_mean = H @ joint_mu
            S = H @ joint_Sigma @ H.T + R_diag
            S = 0.5 * (S + S.T)
            log_like[m] = self._gaussian_logpdf_mvn(y_vec, pred_mean, S)
            ...
            KH = K @ H
            tmp = I - KH
            joint_Sigma_post = tmp @ joint_Sigma @ tmp.T + K @ R_diag @ K.T
    ...
    def update_step(
        ...
        observed_residual: float | np.ndarray,
    ...
    def _mode_residual_stats(
        ...
        if self.d_g == 0:
            for m in range(self.M):
                mu_u_m = mu_u_pred[k][m]
                Sigma_u_m = Sigma_u_pred[k][m]
                mean_modes[m] = phi_T @ mu_u_m
                noise = phi_T @ Sigma_u_m @ phi + self._get_R_matrix(m, k, d_y)
                noise = 0.5 * (noise + noise.T)
                cov_modes[m] = noise
            return mean_modes, cov_modes
        ...
        for m in range(self.M):
            ...
            Sigma_u_m = Sigma_u_pred[k][m]
            H_g = phi_T @ Bk
            mean_modes[m] = H_g @ mu_g_m + phi_T @ mu_u_m
            signal = H_g @ Sigma_g_m @ H_g.T
            signal = 0.5 * (signal + signal.T)
            noise = phi_T @ Sigma_u_m @ phi + self._get_R_matrix(m, k, d_y)
            noise = 0.5 * (noise + noise.T)
            cov_modes[m] = signal + noise
        return mean_modes, cov_modes
    ...
    def _filter_block_nll(
        ...
        for t in range(T):
            ...
            if full_feedback:
                residuals_t = self._as_residual_matrix(residuals_full[t])
                (
                    w_post,
                    mu_g_post,
                    Sigma_g_post,
                ...
                residual = self._as_residual_vector(residuals[t])
                mean_modes, cov_modes = self._mode_residual_stats(
                    phi, mu_g_pred, Sigma_g_pred, mu_u_pred, Sigma_u_pred, action
                )
                loglikes = np.log(np.maximum(w_pred, self.eps))
                for m in range(self.M):
                    loglikes[m] += self._gaussian_logpdf_mvn(
                        residual, mean_modes[m], cov_modes[m]
                    )
                if accumulate:
    ...
    def _kalman_sample(
        ...
        for t in range(T):
            ...
            if obs_mask[t]:
                H_t = np.asarray(H_seq[t], dtype=float)
                y_t = np.asarray(y_seq[t], dtype=float)
                if H_t.ndim == 1:
                    R_t = float(R_seq[t])
                    S = float(H_t @ P_pred[t] @ H_t.T + R_t)
                    S = max(S, self.eps)
                    K = (P_pred[t] @ H_t.T) / S
                    innovation = float(y_t - H_t @ m_pred[t])
                    m_filt[t] = m_pred[t] + (K.flatten() * innovation)
                    P_filt[t] = P_pred[t] - K @ H_t @ P_pred[t]
                    P_filt[t] = 0.5 * (P_filt[t] + P_filt[t].T) + self.eps * np.eye(d)
                else:
                    y_vec = y_t.reshape(-1)
                    n_obs = int(H_t.shape[0])
                    if y_vec.shape[0] != n_obs:
                        raise ValueError("Observation dimension mismatch in Kalman update.")
                    if n_obs == 1:
                        h = H_t.reshape(-1)
                        R_t = R_seq[t]
                        if np.ndim(R_t) == 0:
                            ...
                            if R_arr.shape == (1,):
                                r = float(R_arr[0])
                            elif R_arr.shape == (1, 1):
                                r = float(R_arr[0, 0])
                            else:
                                raise ValueError("R dimension mismatch in Kalman update.")
                        S = float(h @ P_pred[t] @ h.T + r)
                        S = max(S, self.eps)
                        K = (P_pred[t] @ h.T) / S
                        innovation = float(y_vec[0] - h @ m_pred[t])
                        m_filt[t] = m_pred[t] + (K.flatten() * innovation)
                        P_filt[t] = P_pred[t] - np.outer(K, h) @ P_pred[t]
                        P_filt[t] = 0.5 * (P_filt[t] + P_filt[t].T) + self.eps * np.eye(d)
                        continue
                    R_t = R_seq[t]
                    if np.ndim(R_t) == 0:
                        R_mat = np.eye(n_obs, dtype=float) * float(R_t)
                    else:
                        R_arr = np.asarray(R_t, dtype=float)
                        if R_arr.ndim == 1:
                            if R_arr.shape[0] != n_obs:
                                raise ValueError("R dimension mismatch in Kalman update.")
                            R_mat = np.diag(R_arr)
                        else:
                            if R_arr.shape != (n_obs, n_obs):
                                raise ValueError("R dimension mismatch in Kalman update.")
                            R_mat = R_arr
                    S = H_t @ P_pred[t] @ H_t.T + R_mat
                    S = 0.5 * (S + S.T) + self.eps * np.eye(n_obs, dtype=float)
                    PHt = P_pred[t] @ H_t.T
                    try:
    ...
    def _kalman_sample_flexible(
        ...
        for t in range(T):
            ...
            H_t = np.asarray(H_seq[t], dtype=float)
            y_t = np.asarray(y_seq[t], dtype=float).reshape(-1)
            if y_t.size == 0 or H_t.size == 0:
                m_filt[t] = m_pred[t]
                P_filt[t] = P_pred[t]
                continue
            if H_t.ndim == 1:
            ...
            if H_t.shape[0] == 1:
                h = H_t.reshape(-1)
                R_t = np.asarray(R_seq[t], dtype=float)
                if R_t.ndim == 0:
                    r = float(R_t)
                elif R_t.shape == (1,):
                    r = float(R_t[0])
                ...
                S = max(S, self.eps)
                K = (P_pred[t] @ h.T) / S
                innovation = float(y_t[0] - h @ m_pred[t])
                m_filt[t] = m_pred[t] + (K.flatten() * innovation)
                P_filt[t] = P_pred[t] - np.outer(K, h) @ P_pred[t]
                P_filt[t] = 0.5 * (P_filt[t] + P_filt[t].T) + self.eps * np.eye(d)
                continue
            R_t = np.asarray(R_seq[t], dtype=float)
            if R_t.ndim == 0:
                R_mat = np.eye(int(y_t.shape[0]), dtype=float) * float(R_t)
            elif R_t.ndim == 1:
                if R_t.shape[0] != y_t.shape[0]:
                    raise ValueError("R dimension mismatch in Kalman update.")
                R_mat = np.diag(R_t)
            else:
                if R_t.shape != (y_t.shape[0], y_t.shape[0]):
                    raise ValueError("R dimension mismatch in Kalman update.")
                R_mat = R_t
            S = H_t @ P_pred[t] @ H_t.T + R_mat
            S = 0.5 * (S + S.T) + self.eps * np.eye(int(y_t.shape[0]), dtype=float)
            PHt = P_pred[t] @ H_t.T
            try:
                L = np.linalg.cholesky(S)
                tmp = np.linalg.solve(L, PHt.T)
    ...
    def _sample_z_sequence(
        ...
        for t in range(T):
            ...
            scalar_obs = d_y == 1
            if full_feedback:
                residuals_full_t = self._as_residual_matrix(residuals_full[t])
                for k in available_sets[t]:
                    k = int(k)
                    Bk = self._ensure_B(k)
                    mean_shared = phi_T @ (Bk @ g_prev[t] + u_prev[k][t])
                    resid_vec = self._as_residual_vector(residuals_full_t[k])
                    if scalar_obs:
                        diff = float(resid_vec[0] - mean_shared.reshape(-1)[0])
                        R_vals = np.maximum(self._get_R_scalars_for_expert(k), self.eps)
                        log_emission[t] += -0.5 * (
                            np.log(2.0 * np.pi * R_vals) + (diff * diff) / R_vals
                        )
                    else:
                        for m in range(self.M):
                            R_mat = self._get_R_matrix(m, k, d_y)
                            log_emission[t, m] += self._gaussian_logpdf_mvn(
                                resid_vec, mean_shared, R_mat
                            )
            else:
                k = int(actions[t])
                Bk = self._ensure_B(k)
                mean_shared = phi_T @ (Bk @ g_prev[t] + u_prev[k][t])
                resid_vec = self._as_residual_vector(residuals[t])
                if scalar_obs:
                    diff = float(resid_vec[0] - mean_shared.reshape(-1)[0])
                    R_vals = np.maximum(self._get_R_scalars_for_expert(k), self.eps)
                    log_emission[t] = -0.5 * (
    ...
    def fit_em(
        ...
        if self.N is None and expert_ids:
            self.N = int(max(expert_ids) + 1)
        for k in expert_ids:
            self._ensure_B(k)
        ...
        if psi_R_cfg is None:
            ...
            Psi_R = np.asarray(psi_R_cfg, dtype=float)
            if Psi_R.shape == ():
                Psi_R = np.eye(d_y, dtype=float) * float(Psi_R)
            elif Psi_R.shape != (d_y, d_y):
                raise ValueError("Psi_R must be scalar or (d_y, d_y).")
        nu_R = float(priors.get("nu_R", 0.0))
        ...
        for em_idx in range(n_em):
            ...
            phi_vec_seq = None
            if phi_seq and int(phi_seq[0].shape[1]) == 1:
                phi_vec_seq = np.stack([phi[:, 0] for phi in phi_seq], axis=0)
            ...
            for s in range(total_samples):
                ...
                if full_feedback:
                    ...
                    for t in range(len(train_ctx)):
                        ...
                        R_blocks = []
                        y_vals = []
                        residuals_full_t = self._as_residual_matrix(train_residuals_full[t])
                        z_m = int(z_seq[t])
                        for k in train_avail[t]:
                            ...
                            H_g = phi_T @ Bk
                            resid_vec = self._as_residual_vector(residuals_full_t[k])
                            y_vec = resid_vec - phi_T @ u_prev[k][t]
                            H_rows.append(H_g)
                            R_blocks.append(self._get_R_matrix(z_m, k, d_y))
                            y_vals.append(y_vec)
                        if H_rows:
                            H_t = np.vstack(H_rows)
                            y_t = np.concatenate(y_vals)
                            R_t = np.zeros((y_t.shape[0], y_t.shape[0]), dtype=float)
                            for idx, Rk in enumerate(R_blocks):
                                start = idx * d_y
                                R_t[start : start + d_y, start : start + d_y] = Rk
                        else:
                            H_t = np.zeros((0, self.d_g), dtype=float)
                            y_t = np.zeros(0, dtype=float)
                            R_t = np.zeros((0, 0), dtype=float)
                        H_seq_g.append(H_t)
                        R_seq_g.append(R_t)
                        y_seq_g.append(y_t)
                    g_seq = self._kalman_sample_flexible(
                        A_seq_g,
                        Q_seq_g,
                        H_seq_g,
                        R_seq_g,
                    ...
                    H_seq_g = np.zeros((len(train_ctx), d_y, self.d_g), dtype=float)
                    R_seq_g = np.zeros((len(train_ctx), d_y, d_y), dtype=float)
                    y_seq_g = np.zeros((len(train_ctx), d_y), dtype=float)
                    ...
                    for t in range(len(train_ctx)):
                        ...
                        Bk = self._ensure_B(k)
                        H_seq_g[t] = phi_T @ Bk
                        R_seq_g[t] = self._get_R_matrix(int(z_seq[t]), k, d_y)
                        resid_vec = self._as_residual_vector(train_residuals[t])
                        y_seq_g[t] = resid_vec - phi_T @ u_prev[k][t]
                ...
                for k in expert_ids:
                    ...
                    H_seq_u = np.zeros((len(train_ctx), d_y, self.d_phi), dtype=float)
                    R_seq_u = np.zeros((len(train_ctx), d_y, d_y), dtype=float)
                    y_seq_u = np.zeros((len(train_ctx), d_y), dtype=float)
                    ...
                    for t in range(len(train_ctx)):
                        if full_feedback:
                            if k not in train_avail[t]:
                                continue
                            resid_val = self._as_residual_vector(
                                self._as_residual_matrix(train_residuals_full[t])[k]
                            )
                        else:
                            if int(train_actions[t]) != k:
                                continue
                            resid_val = self._as_residual_vector(train_residuals[t])
                        phi_t = phi_seq[t]
                        phi_T = phi_t.T
                        H_seq_u[t] = phi_T
                        R_seq_u[t] = self._get_R_matrix(int(z_seq[t]), k, d_y)
                        y_seq_u[t] = resid_val - phi_T @ (Bk @ g_seq[t])
                        obs_mask_u[t] = True
            ...
            z_next_flat = z_samples[:, 1:].reshape(-1) if T_train > 1 else None
            if self.d_g > 0 and T_train > 1:
                g_curr_flat = g_samples[:, 1:, :].reshape(-1, self.d_g)
                g_prev_flat = g_samples[:, :-1, :].reshape(-1, self.d_g)
            else:
                g_curr_flat = None
                g_prev_flat = None
            ...
            for m in range(self.M):
                if self.d_g == 0:
                    continue
                if z_next_flat is None or g_curr_flat is None or g_prev_flat is None:
                    continue
                mask = z_next_flat == m
                count_raw = int(np.count_nonzero(mask))
                count = float(count_raw) / S
                if count <= epsilon_N:
                    continue
                Gc = g_curr_flat[mask]
                Gp = g_prev_flat[mask]
                ...
                diff = Gc - (Gp @ self.A_g[m].T)
                sum_gres = (diff.T @ diff) / S
            ...
            for m in range(self.M):
                ...
                for k in expert_ids:
                    ...
                    if T_train <= 1:
                        continue
                    u_curr = u_samp[:, 1:, :].reshape(-1, self.d_phi)
                    u_prev_flat_k = u_samp[:, :-1, :].reshape(-1, self.d_phi)
                    mask = z_next_flat == m
                    count_raw = int(np.count_nonzero(mask))
                    if count_raw == 0:
                        continue
                    Uc = u_curr[mask]
                    Up = u_prev_flat_k[mask]
                    sum_uu += Uc.T @ Up
                    sum_uprev += Up.T @ Up
                ...
                for k in expert_ids:
                    ...
                    if not np.any(mask):
                        continue
                    diff = u_curr[mask] - (u_prev_flat_k[mask] @ self.A_u[m].T)
                    sum_ures += diff.T @ diff
                sum_ures /= S
            ...
            if self.d_g > 0 and self.d_phi > 0:
                for k in expert_ids:
                    ...
                    if d_y == 1 and phi_vec_seq is not None and obs_idx.size > 0:
                        ...
                        w_flat = 1.0 / R_flat
                        XTX += (X.T @ (X * w_flat[:, None])) / S
                        XTy += (X.T @ (y_flat * w_flat)) / S
                    else:
                        for t in obs_idx:
                            if full_feedback:
                                resid_val = self._as_residual_vector(
                            ...
                            for s in range(z_samples.shape[0]):
                                ...
                                u_t = u_samples[k][s, t]
                                R_mat = self._get_R_matrix(m, k, d_y)
                                R_mat = 0.5 * (R_mat + R_mat.T) + self.eps * np.eye(d_y)
                                y_vec = resid_val - phi_T @ u_t
                                X = np.stack(
                                    [np.kron(phi_t[:, r], g_t) for r in range(d_y)],
                                    axis=0,
                                )
                                try:
                                    X_weighted = np.linalg.solve(R_mat, X)
                                    y_weighted = np.linalg.solve(R_mat, y_vec)
                                except np.linalg.LinAlgError:
                                    R_mat = R_mat + self.eps * np.eye(d_y)
                                    X_weighted = np.linalg.solve(R_mat, X)
                                    y_weighted = np.linalg.solve(R_mat, y_vec)
                                weight = 1.0 / S
                                XTX += weight * (X.T @ X_weighted)
                                XTy += weight * (X.T @ y_weighted)
            ...
            # Update R_{m,k}
            if self.R_mode == "full":
                self._ensure_full_R(d_y)
                for m in range(self.M):
                    for k in expert_ids:
                        sum_res = np.zeros((d_y, d_y), dtype=float)
                        denom = 0.0
                        for t in range(T_train):
                            if full_feedback:
                                if k not in train_avail[t]:
                            ...
                            for s in range(z_samples.shape[0]):
                                ...
                                g_t = g_samples[s, t]
                                u_t = u_samples[k][s, t]
                                resid_vec = resid_val - phi_T @ (Bk @ g_t + u_t)
                                sum_res += np.outer(resid_vec, resid_vec) / S
                                denom += 1.0 / S
                        if denom <= epsilon_N:
                            continue
                        self.R[m, k] = (Psi_R + sum_res) / max(
                            nu_R + denom, self.eps
                        )
            else:
                if np.ndim(self.R) == 0:
                        ...
                        if d_y == 1 and phi_vec_seq is not None:
                            ...
                            if obs_idx.size > 0:
                                ...
                                pred = np.einsum(
                                    "ti,sti->st",
                                    phi_obs,
                                    np.einsum("ij,stj->sti", Bk, g_obs) + u_obs,
                                )
                                diff_sq = (resid_obs[None, :] - pred) ** 2
                                z_flat = z_samples[:, obs_idx].reshape(-1)
                                diff_sq_flat = diff_sq.reshape(-1)
                                num = float(np.sum(diff_sq_flat[z_flat == m])) / S
                                denom = float(np.count_nonzero(z_flat == m)) / S
                        else:
                            for t in range(T_train):
                                if full_feedback:
                                    if k not in train_avail[t]:
                                ...
                                for s in range(z_samples.shape[0]):
                                    ...
                                    u_t = u_samples[k][s, t]
                                    resid_vec = resid_val - phi_T @ (Bk @ g_t + u_t)
                                    num += float(resid_vec @ resid_vec) / S
                                    denom += float(d_y) / S
                        if denom <= epsilon_N:
                            continue
