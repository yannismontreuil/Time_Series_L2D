#!/bin/bash
#SBATCH --job-name=jena_trade
#SBATCH --partition=long
#SBATCH --output=logs/jena_trade_%j.out
#SBATCH --error=logs/jena_trade_%j.err
#SBATCH --nodes=1
#SBATCH --mem=64G
...
HORIZON_ARG=""
if [[ -n "${JENA_TRADEOFF_HORIZON:-}" ]]; then
  HORIZON_ARG="--horizon ${JENA_TRADEOFF_HORIZON}"
fi

NAMES_ARG=""
if [[ -n "${JENA_TRADEOFF_NAMES:-}" ]]; then
  NAMES_ARG="--names ${JENA_TRADEOFF_NAMES}"
fi

REPEATS="${JENA_TRADEOFF_REPEATS:-2}"
OUT_CSV="${JENA_TRADEOFF_OUT:-out/jena_tradeoff_sweep_${SLURM_JOB_ID}.csv}"

echo "Running Jena tradeoff sweep..."
echo "Horizon override: ${JENA_TRADEOFF_HORIZON:-full}"
echo "Repeats: ${REPEATS}"
echo "Candidates: ${JENA_TRADEOFF_NAMES:-all}"
echo "Output CSV: ${OUT_CSV}"

FACTOR_DISABLE_PLOT_SHOW=1 MPLBACKEND=Agg \
  "${PYTHON}" -u scripts/jena_tradeoff_sweep.py \
  --repeats "${REPEATS}" \
  --output "${OUT_CSV}" \
  ${HORIZON_ARG} \
  ${NAMES_ARG}
