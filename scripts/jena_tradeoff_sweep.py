...
from scripts.measure_jena_runtime import (
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Sweep Jena L2D-SLDS tradeoffs between cost and runtime.")
    parser.add_argument(
        "--config",
        type=pathlib.Path,
        default=ROOT / "config" / "config_jena_tuned.yaml",
        help="Base Jena config.",
    )
    parser.add_argument(
        "--horizon",
        type=int,
        default=None,
        help="Optional horizon override for faster smoke checks.",
    )
    parser.add_argument(
        "--repeats",
        type=int,
        default=2,
        help="How many timing repeats to average per candidate.",
    )
    parser.add_argument(
        "--names",
        nargs="*",
        default=None,
        help="Optional candidate names to evaluate.",
    )
    parser.add_argument(
        "--output",
        type=pathlib.Path,
        default=ROOT / "out" / "jena_tradeoff_sweep.csv",
...
def _configure_candidate(base_cfg: dict[str, Any], cand: dict[str, Any], horizon: int | None) -> dict[str, Any]:
...
def main() -> None:
    ...
    for cand in candidates:
        ...
        rows.append(row)
        print(
            f"{row['name']}: avg={row['avg_cost']:.4f}, cum={row['cumulative_cost']:.2f}, "
    ...
    for row in sorted(rows, key=lambda r: (r["avg_cost"], r["ms_per_step"])):
        if row["pareto"]:
            print(
                ...
                f"cum={row['cumulative_cost']:.2f}, time={row['ms_per_step']:.3f} ms/step"
            )
