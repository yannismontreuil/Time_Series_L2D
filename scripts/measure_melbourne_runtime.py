...
def _apply_factorized_init(env: ETTh1TimeSeriesEnv, routers: list[FactorizedSLDS], factor_cfg: dict) -> None:
    ...
    if init_mode not in ("none", "off", "false"):
        window = int(factor_cfg.get("init_R_window", min(730, env.T - 1)))
    ...
    window = int(
        factor_cfg.get(
            "init_state_window",
            factor_cfg.get("init_R_window", min(730, env.T - 1)),
        )
    )
...
def _build_factorized_pair(cfg: dict[str, Any], env: ETTh1TimeSeriesEnv):
    env_cfg = cfg["environment"]
...
def _build_baselines(cfg: dict[str, Any], env: ETTh1TimeSeriesEnv):
    env_cfg = cfg["environment"]
...
def main() -> None:
    ...
    try:
        ...
        ours, no_g = _build_factorized_pair(cfg_runtime, env)
        linucb, shared, neural, lints, ensemble = _build_baselines(cfg_runtime, env)

        rows = [
            _measure("L2D-SLDS", run_factored_router_on_env, ours, env),
            _measure("L2D-SLDS w/o $g_t$", run_factored_router_on_env, no_g, env),
