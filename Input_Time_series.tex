...
To encode cross-expert dependence parsimoniously, we use a shared-factor decomposition and allow the regime transitions to
depend on the observed context $\mathbf x_t$ via a neural network.

Let
\[
    z_t \in \mathcal Z \coloneqq \{1,\dots,M\}
\]
denote the latent regime at time $t$ (e.g., \emph{normal operations} vs.\ \emph{outbreak}). Instead of a time-homogeneous
transition matrix, we assume $z_t$ follows a \emph{context-dependent} Markov chain:
for $t\ge2$,
\begin{equation}
    \label{eq:regime_transition}
    \mathbb{P}\!\left(z_t=k \mid z_{t-1}=i,\mathbf x_t\right)
    \;=\;
    \Pi_\theta(i,k\mid \mathbf x_t),
    \qquad
    \sum_{k=1}^M  \Pi_\theta(i,k\mid \mathbf x_t)=1 \;\;\; \forall\, i\in\{1,\dots,M\}.
\end{equation}
Here $\Pi_\theta(\cdot,\cdot\mid \mathbf x_t)$ is a row-stochastic transition matrix produced by a neural network with
parameters $\theta$. To avoid explicitly predicting $M^2$ independent logits, we parameterize the transition scores via a
low-rank ``attention'' factorization. Let $Q(\mathbf x_t),K(\mathbf x_t)\in\mathbb R^{M\times d}$ be two matrices whose
rows are context-dependent query and key vectors for each regime. Define the score matrix
\[
    S(\mathbf x_t) \;\coloneqq\; \frac{Q(\mathbf x_t)K(\mathbf x_t)^\top}{\sqrt d}
    \in\mathbb R^{M\times M},
\]
so that $S_{ik}(\mathbf x_t)$ measures the compatibility between regime $i$ at time $t-1$ and regime $k$ at time $t$ given
the current context. We then set
\begin{equation}
    \label{eq:attn_transition}
    \Pi_\theta(i,k\mid \mathbf x_t)
    \;=\;
    \frac{\exp\bigl(S_{ik}(\mathbf x_t)\bigr)}
         {\sum_{k'=1}^M \exp\bigl(S_{ik'}(\mathbf x_t)\bigr)}
    \;=\;
    \operatorname{softmax}_k\!\left( \frac{Q(\mathbf x_t) K(\mathbf x_t)^\top}{\sqrt{d}} \right)_{ik},
\end{equation}
where $\operatorname{softmax}_k$ denotes the row-wise softmax over $k$. This yields a valid Markov kernel for each realized
context $\mathbf x_t$, with logits constrained to a low-rank manifold via the shared query/key representations.
...
\draw[param_edge] (xt) -- node[midway, right, font=\tiny, xshift=2pt] {$\Pi_\theta(\cdot,\cdot\mid \mathbf x_t)$} (zt);
...
Define the predicted (one-step) regime prior at decision time $t+1$, conditional on the newly observed context $\mathbf x_{t+1}$,
\[
    \bar b_{t+1\mid t}(k)\;\coloneqq\;\mathbb P(z_{t+1}=k\mid \mathcal I_t,\mathbf x_{t+1})
    \;=\;
    \sum_{i=1}^M b_t(i)\,\Pi_\theta(i,k\mid \mathbf x_{t+1}).
\]
For each candidate next regime $k$, define the IMM mixing weights
\[
    \mu_{i\mid k}
    \;\coloneqq\;
    \mathbb P(z_t=i\mid z_{t+1}=k,\mathcal I_t,\mathbf x_{t+1})
    \;=\;
    \frac{\Pi_\theta(i,k\mid \mathbf x_{t+1})\,b_t(i)}{\bar b_{t+1\mid t}(k)}.
\]
...
Because the regime transition kernel now depends on the current context via $\Pi_\theta(\cdot,\cdot\mid \mathbf x_{t+1})$,
the predicted regime probabilities at decision time $t+1$ are
\[
    \bar b_{t+1\mid t}(k)
    \;\coloneqq\;
    \mathbb P(z_{t+1}=k\mid \mathcal I_t,\mathbf x_{t+1})
    \;=\;
    \sum_{i=1}^M b_t(i)\,\Pi_\theta(i,k\mid \mathbf x_{t+1}),
    \qquad k\in\mathcal Z.
\]
...
We treat the feature map $\phi:\mathcal X\to\mathbb R^{d_\alpha}$ and the loadings $(B_j)$ as fixed, and learn:
\begin{itemize}
    \item the linear-Gaussian SLDS parameters
    \[
        \Theta_{\mathrm{lin}}
        \;\coloneqq\;
        \Bigl(
            \mu^0,\ (A^{(g)}_k,Q^{(g)}_k)_{k=1}^M,\ (A^{(u)}_k,Q^{(u)}_k)_{k=1}^M,\
            (R_{k,j})_{\substack{k=1,\dots,M\\ j\in\mathcal U_T}}
        \Bigr),
    \]
    governing the latent dynamics and observation noise; and
    \item the parameters $\theta$ of the context-dependent transition network $\Pi_\theta(i,k\mid\mathbf x_t)$ defined in
          \eqref{eq:regime_transition}--\eqref{eq:attn_transition}.
\end{itemize}
For clarity, we write $\Theta \coloneqq (\Theta_{\mathrm{lin}},\theta)$ when referring to the full parameter set. The
components of $\Theta_{\mathrm{lin}}$ are defined in
...
\paragraph{Regime transitions (context-dependent).}
For a time-homogeneous Markov chain with fixed transition matrix $\Pi$, the M-step update would be
\[
    \Pi_{ik}^{(m+1)}
    \;=\;
    \frac{\sum_{t=2}^T \xi_t^{(i,k)}}{\sum_{t=2}^T \sum_{k'=1}^M \xi_t^{(i,k')}},
    \qquad i,k\in\mathcal Z.
\]
In our context-dependent model, the regime kernel is instead parameterized by a neural network as
in \eqref{eq:attn_transition}, so there is no closed-form M-step. The contribution of transitions to the generalized-EM
objective is
\[
    Q_{\mathrm{trans}}(\theta)
    \;\coloneqq\;
    \sum_{t=2}^T \sum_{i=1}^M \sum_{k=1}^M
        \xi_t^{(i,k)} \,\log \Pi_\theta(i,k\mid \mathbf x_t),
\]
which is a weighted cross-entropy between the posterior transition counts $\xi_t^{(i,k)}$ and the model probabilities
$\Pi_\theta(i,k\mid \mathbf x_t)$. Using the attention parametrization \eqref{eq:attn_transition}, we optimize
$Q_{\mathrm{trans}}(\theta)$ by gradient-based methods rather than by a closed-form update.
...
\[
    \bar c_t(k)
    \;\coloneqq\;
    \mathbb P(z_t=k\mid \mathcal I_{t-1}^{\mathrm{full}},\mathbf x_t)
    \;=\;
    \sum_{i=1}^M b_{t-1}(i)\,\Pi_\theta(i,k\mid \mathbf x_t).
\]
Bayes' rule yields
\[
    b_t(k)
...
\]
computed by iterating the context-dependent transition kernel forward without conditioning on future losses. In the
time-homogeneous case this reduces to $\pi_{t+h\mid t} = \pi_t \Pi^{h}$; with context-dependent transitions, the forward
law uses $\Pi_\theta(\cdot,\cdot\mid \mathbf x_{t+1}),\dots,\Pi_\theta(\cdot,\cdot\mid \mathbf x_{t+h})$ or a suitable
scenario model for future contexts (Section~\ref{sec:staffing-context-forecast}).
...
IMM+star belief at time $t$: $\pi_t$ and star moments; context-dependent regime kernel $\Pi_\theta(\cdot,\cdot\mid\cdot)$;
...
\For{$h=1$ to $H$}
    \State Propagate $\pi_{t+h\mid t}$ using the context-dependent kernel (Section~\ref{sec:staffing-open-loop}).
...
Section~\ref{sec:param-opt} (for $\mu^0,A_k^{(g)},Q_k^{(g)},A_k^{(u)},Q_k^{(u)},R_{k,j},\theta$ and the birth/initial priors),
