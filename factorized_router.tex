...
\paragraph{Full-feedback variant.}
If full feedback is available, we observe residuals \(\{e_{t,k}\}_{k\in\mathcal{E}_t}\) at time \(t\).
For each mode \(m\), we apply sequential Kalman corrections for each \(k\in\mathcal{E}_t\) using the
observation model
\[
e_{t,k}=h_{t,k}^\top \mathbf{g}_t + \phi(\mathbf{x}_t)^\top \mathbf{u}_{t,k} + \varepsilon_{t,k},
\qquad \varepsilon_{t,k}\sim\mathcal{N}(0,R_{m,k}),
\]
where \(h_{t,k}\coloneqq \mathbf{B}_k^\top \phi(\mathbf{x}_t)\).
Let \((\mu^{(m)}_{g,t|t-1},\Sigma^{(m)}_{g,t|t-1})\) and
\((\mu^{(m)}_{u,k,t|t-1},\Sigma^{(m)}_{u,k,t|t-1})\) be the predictive marginals. The innovation mean
and variance for expert \(k\) are
\[
\mu^{(m)}_{t,k}=\phi(\mathbf{x}_t)^\top\!\left(\mathbf{B}_k\mu^{(m)}_{g,t|t-1}+\mu^{(m)}_{u,k,t|t-1}\right),
\]
\[
S^{(m)}_{t,k}=\phi(\mathbf{x}_t)^\top\!\left(\mathbf{B}_k\Sigma^{(m)}_{g,t|t-1}\mathbf{B}_k^\top
+\Sigma^{(m)}_{u,k,t|t-1}\right)\phi(\mathbf{x}_t)+R_{m,k}.
\]
We update \(\mathbf{g}_t\) and \(\mathbf{u}_{t,k}\) via standard Kalman gains and keep the remaining
\(\mathbf{u}_{t,\ell}\) at their predictive marginals. The mode likelihood is the product of
innovation terms,
\(
\mathcal{L}_t^{(m)}=\prod_{k\in\mathcal{E}_t}\mathcal{N}\!\left(e_{t,k};\mu^{(m)}_{t,k},S^{(m)}_{t,k}\right),
\)
and the regime weights are updated as above. We retain the factorized approximation by discarding
cross-covariances between \(\mathbf{g}_t\) and \(\mathbf{u}_{t,k}\) after each correction.
...
% =========================
\begin{algorithm}[H]
\caption{\textsc{LearnParameter\_EM}: Monte Carlo EM for the Factorized SLDS (includes $\theta$ for context-dependent transitions)}
\label{alg:trainmodel_em}
\begin{algorithmic}[1]
\STATE {\bfseries Input:} contexts $(\mathbf{x}_t)_{t=1}^T$; available sets $(\mathcal{E}_t)_{t=1}^T$; actions $(I_t)_{t=1}^T$; observed residuals $(e_t)_{t=1}^T$ with $e_t=\widehat{y}_{t,I_t}-y_t$; feature map $\phi$; EM iterations $N_{\mathrm{EM}}$; posterior samples $N_{\mathrm{samp}}$; burn-in $N_{\mathrm{burn}}$; MAP priors $(M^{(g)}_A,\lambda^{(g)}_A,\Psi^{(g)},\nu^{(g)})$, $(M^{(u)}_A,\lambda^{(u)}_A,\Psi^{(u)},\nu^{(u)})$, $(M_B,\lambda_B)$, $(a_R,b_R)$, and $\lambda_\theta$ for the attention-based transition parameters $\theta$ in $\Pi_\theta(\mathbf{x}_t)$.
\STATE $\mathcal{K}\leftarrow \bigcup_{t=1}^T \mathcal{E}_t$
\STATE {\bfseries Initialize:} parameters $\Theta^{(0)}$; initial state priors for $z_1,\mathbf{g}_1,\{\mathbf{u}_{1,k}\}_{k\in\mathcal{K}}$.
\FOR{iteration $i = 1$ to $N_{\mathrm{EM}}$}
    \STATE \textbf{// E-STEP: Monte Carlo posterior}
    \STATE Initialize $(z_{1:T},\mathbf{g}_{1:T},\{\mathbf{u}_{1:T,k}\}_{k\in\mathcal{K}})$ from previous samples.
    \FOR{sample $s=1$ to $N_{\mathrm{burn}}+N_{\mathrm{samp}}$}
        \STATE Sample $z_{1:T}^{(s)}$ via FFBS using $\Pi_\theta(\mathbf{x}_t)$ and the emission likelihood of $e_t$ conditional on $(\mathbf{g}^{(s-1)}_{1:T},\{\mathbf{u}^{(s-1)}_{1:T,k}\}_k)$.
        \STATE Sample $\mathbf{g}_{1:T}^{(s)}$ via Kalman smoothing given $z_{1:T}^{(s)}$, $\{\mathbf{u}^{(s-1)}_{1:T,k}\}_k$, and the bandit observations $\{e_t\}$ (missing for $k\neq I_t$).
        \FOR{each expert $k\in\mathcal{K}$}
            \STATE Sample $\mathbf{u}_{1:T,k}^{(s)}$ via Kalman smoothing given $z_{1:T}^{(s)}$, $\mathbf{g}^{(s)}_{1:T}$, and the observations $\{e_t: I_t=k\}$ (missing otherwise).
        \ENDFOR
        \IF{$s>N_{\mathrm{burn}}$}
    ...
    \STATE For each mode $m$, update
    $A_m^{(g)}\leftarrow\left(S^{(m)}_{g g^-}+\lambda^{(g)}_A M^{(g)}_A\right)\left(S^{(m)}_{g^- g^-}+\lambda^{(g)}_A I\right)^{-1}$,
    $Q_m^{(g)}\leftarrow\frac{\Psi^{(g)}+S^{(m)}_{g\mid g^-}}{\nu^{(g)}+N_m+d_g+1}$,
    where $g^-$ denotes $g_{t-1}$,
    $S^{(m)}_{g g^-}\!=\!\sum_t\gamma_t^{(m)}\mathbb{E}[\mathbf{g}_t\mathbf{g}_{t-1}^\top]$,
    $S^{(m)}_{g^- g^-}\!=\!\sum_t\gamma_t^{(m)}\mathbb{E}[\mathbf{g}_{t-1}\mathbf{g}_{t-1}^\top]$,
    $S^{(m)}_{g\mid g^-}\!=\!\sum_t\gamma_t^{(m)}\mathbb{E}[(\mathbf{g}_t-A_m^{(g)}\mathbf{g}_{t-1})(\mathbf{g}_t-A_m^{(g)}\mathbf{g}_{t-1})^\top]$,
    and $N_m=\sum_t\gamma_t^{(m)}$.
    \STATE For each mode $m$, update
    $A_m^{(u)}\leftarrow\left(S^{(m)}_{u u^-}+\lambda^{(u)}_A M^{(u)}_A\right)\left(S^{(m)}_{u^- u^-}+\lambda^{(u)}_A I\right)^{-1}$,
    $Q_m^{(u)}\leftarrow\frac{\Psi^{(u)}+S^{(m)}_{u\mid u^-}}{\nu^{(u)}+N^{(u)}_m+d_u+1}$,
    where $u^-$ denotes $u_{t-1,k}$,
    $S^{(m)}_{u u^-}\!=\!\sum_{k\in\mathcal{K}}\sum_t\gamma_t^{(m)}\mathbb{E}[\mathbf{u}_{t,k}\mathbf{u}_{t-1,k}^\top]$,
    $S^{(m)}_{u^- u^-}\!=\!\sum_{k\in\mathcal{K}}\sum_t\gamma_t^{(m)}\mathbb{E}[\mathbf{u}_{t-1,k}\mathbf{u}_{t-1,k}^\top]$,
    $S^{(m)}_{u\mid u^-}\!=\!\sum_{k\in\mathcal{K}}\sum_t\gamma_t^{(m)}\mathbb{E}[(\mathbf{u}_{t,k}-A_m^{(u)}\mathbf{u}_{t-1,k})(\mathbf{u}_{t,k}-A_m^{(u)}\mathbf{u}_{t-1,k})^\top]$,
    and $N^{(u)}_m=\sum_{k\in\mathcal{K}}\sum_t\gamma_t^{(m)}$.
    \STATE For each expert $k$, update $\mathbf{B}_k$ by weighted ridge regression on times $\{t:I_t=k\}$:
    let $x_t=\mathbb{E}[\mathbf{g}_t]\otimes\phi(\mathbf{x}_t)$,
    $y_t=e_t-\phi(\mathbf{x}_t)^\top\mathbb{E}[\mathbf{u}_{t,k}]$,
    and $w_t=\sum_{m=1}^M \gamma_t^{(m)}/R_{m,k}$, then
    \[
    \mathrm{vec}(\mathbf{B}_k)\leftarrow\left(\sum_t w_t x_t x_t^\top+\lambda_B I\right)^{-1}\left(\sum_t w_t x_t y_t+\lambda_B\,\mathrm{vec}(M_B)\right).
    \]
    \STATE Update $R_{m,k}$ with an inverse-Gamma MAP estimate (prior $p(R)\propto R^{-(a_R+1)}e^{-b_R/R}$) using
    \[
    R_{m,k}\leftarrow\frac{b_R+\tfrac12\sum_{t:I_t=k}\gamma_t^{(m)}\,r_{t,k}^{(m)2}}{a_R+\tfrac12 N_{m,k}+1},
    \qquad
    r_{t,k}^{(m)}\coloneqq e_t-\phi(\mathbf{x}_t)^\top(\mathbf{B}_k\mathbb{E}_m[\mathbf{g}_t]+\mathbb{E}_m[\mathbf{u}_{t,k}]),
    \]
    where $N_{m,k}=\sum_{t:I_t=k}\gamma_t^{(m)}$.
    \STATE Update transition parameters $\theta$ (the attention logits in $\Pi_\theta(\mathbf{x}_t)$) by maximizing
    $\sum_{t}\sum_{\ell,m}\xi_{t-1}^{(\ell,m)}\log \Pi_{\ell\to m}(\mathbf{x}_t)-\frac{\lambda_\theta}{2}\lVert\theta\rVert_2^2$
...
\begin{algorithmic}[1]
\STATE {\bfseries Input:} horizon $T$; parameters $\Theta$; feature map $\phi$; contexts $(\mathbf{x}_t)_{t=1}^T$; expert sets $(\mathcal{E}_t)_{t=1}^T$; loss $\psi$; fees $(\beta_k)_k$; side information for entering experts (optional).
...
\STATE {\bfseries Hyperparameters:} population priors $(\mu^{(m)}_{\mathrm{pop}},\Sigma^{(m)}_{\mathrm{pop}})_{m=1}^M$; staleness $\Delta_{\max}$.
\FOR{$t=1$ to $T$}
    ...
    \STATE $(\bar w_t,\{\bar\mu^{(m)}_{g,t-1},\bar\Sigma^{(m)}_{g,t-1}\},\{\bar\mu^{(m)}_{u,k,t-1},\bar\Sigma^{(m)}_{u,k,t-1}\}_{k\in\mathcal{K}^{\text{keep}}_t}) \leftarrow \textsc{IMM}(\mathbf{x}_t,w_{t-1},\{\mu^{(m)}_{g,t-1|t-1},\Sigma^{(m)}_{g,t-1|t-1}\},\{\mu^{(m)}_{u,k,t-1|t-1},\Sigma^{(m)}_{u,k,t-1|t-1}\}_{k\in\mathcal{K}^{\text{keep}}_t})$
...
\begin{algorithmic}[1]
\STATE {\bfseries Input:} context $\mathbf{x}_t$; mode weights $w_{t-1}$; global posteriors $(\mu^{(i)}_{g,t-1|t-1},\Sigma^{(i)}_{g,t-1|t-1})$; idiosyncratic posteriors $(\mu^{(i)}_{u,k,t-1|t-1},\Sigma^{(i)}_{u,k,t-1|t-1})$ for $k\in\mathcal{K}^{\text{keep}}_t$.
...
\STATE Mixing weights: $\mu_{t-1}^{i|j} \leftarrow \Pi_{i\to j}(\mathbf{x}_t)\, w_{t-1}^i / \bar w_t^j$
\FOR{$j=1$ to $M$}
