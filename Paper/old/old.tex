...
queried expert. Computing or approximating \eqref{eq:bayes_selection} therefore requires a
...
\)
with $\mathcal{S}_{t,h}\subseteq \mathcal{K}_t$, where $\mathcal{S}_{t,h}$ is the set of experts
kept on-call for time $t+h$. Our goal is to choose $\mathcal{S}_{t:t+H}$ so that, for each $h$, the
planner's predicted choices at time $t+h$ are distilled into a probability-of-use score for each expert.
We then keep on-call the experts whose probability is close to the most likely expert at that time.
Concretely, with tolerance $\delta\in[0,1)$, we include any expert whose time-marginal probability is
at least a $(1-\delta)$ fraction of the maximum; larger $\delta$ lowers the relative threshold and
produces larger sets. We distinguish the ideal schedule
...
$(\mathcal{A}_{t+h})_{h=1}^{H}$, where $\mathcal{A}_{t+h}$ is the set of expert identities known at
time $t$ to be feasible at future time $t+h$.
Since the planner requires a predictive belief for each candidate expert, we restrict to those with
maintained idiosyncratic marginals at time $t$, i.e. the registry \(\mathcal{K}_t\):
\begin{equation}
\label{eq:feas_set}
\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}
\coloneqq
\mathcal{K}_t \cap \mathcal{A}_{t+h},
\qquad h=1,\dots,H.
\end{equation}
Experts outside \(\mathcal{K}_t\) are treated as infeasible even if they are available,
because the planner has no maintained predictive belief for them.
...
If \(\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}=\varnothing\), then no expert is known feasible at time
\(t+h\); in this case we define the planned choice as \(\widehat{I}_{t+h}=\varnothing\) and set the
on-call set \(\mathcal{S}_{t,h}(\delta)=\varnothing\).
In many settings, $(\mathcal{A}_{t+h})_{h=1}^H$ is deterministic and known at time $t$ (e.g., fixed
availability windows), so $\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}$ is a deterministic constraint.
...
$\vhaty_{t+h,k}$ is revealed) and \emph{without} observing the future target $\vy_{t+h}$; it depends
only on the time-$t$ SLDS belief and the hypothesized context prefix. In particular, the belief is
propagated forward \emph{open-loop} using the learned dynamics and the forecasted contexts, and no
measurement updates or exploration bonuses are applied during planning.
...
\end{equation}
with a fixed deterministic tie-break; if \(\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}=\varnothing\),
we set \(\widehat{I}_{t+h}=\varnothing\). The corresponding planned path is
\begin{equation}
\label{eq:pred_path}
...
\end{equation}
We view the planned path as an element of
\(\prod_{h=1}^{H}\left(\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}\cup\{\varnothing\}\right)\), with
\(\widehat{I}_{t+h}=\varnothing\) when \(\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}=\varnothing\).
...
$\mathbf{i}\in\prod_{h=1}^{H}\left(\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}\cup\{\varnothing\}\right)$,
\begin{equation}
\label{eq:path_dist}
...
$H\times|\mathcal{K}_t|$ demand matrix; each row is supported on the feasible set
...
If \(\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}=\varnothing\), then \(\rho_{t,h}(k)=0\) for all
\(k\in\mathcal{K}_t\).
...
step $h\in\{1,\dots,H\}$, we restrict attention to experts that are (i) in the maintained registry at
...
planning}: it does not query any expert during the lookahead (so no $\vhaty_{t+h,k}$ is
revealed), does not observe the future target $\vy_{t+h}$, and performs no measurement updates inside
...
\in
\prod_{h=1}^{H}\left(\mathcal{E}^{\mathrm{feas}}_{t+h \mid t}\cup\{\varnothing\}\right).
\end{equation}
...
\end{equation}
For fixed $h$ and non-empty $\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}$, the vector
$\bigl(\hat{\rho}_{t,h}(k)\bigr)_{k\in\mathcal{K}_t}$ is a pmf supported on
$\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}$: it assigns non-zero mass to multiple experts exactly when
different scenarios yield different planned winners at time $t+h$. If
$\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}=\varnothing$, then $\hat{\rho}_{t,h}(k)=0$ for all $k$.
Conditioned on $\mathcal{F}_t$, the conditional law of large numbers gives
...
the forecast. To translate these marginals into an on-call decision, we form a
(possibly multi-expert) \emph{relative-thresholded active set} by keeping only experts whose marginal
probability is within a multiplicative gap of the most likely expert.

Fix a tolerance $\delta\in[0,1)$. For each $h\in\{1,\dots,H\}$, define the maximum mass
\(
\rho^{\max}_{t,h}\coloneqq \max_{k\in\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}} \rho_{t,h}(k)
\),
with the convention that $\rho^{\max}_{t,h}=0$ when \(\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}=\varnothing\).
The active set is
\begin{equation}
...
\end{equation}
If \(\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}=\varnothing\), then \(\mathcal{S}_{t,h}(\delta)=\varnothing\).
Otherwise, $\mathcal{S}_{t,h}(\delta)$ always contains the maximizers of $\rho_{t,h}$ and expands as the
distribution flattens (larger $\delta$ lowers the relative threshold). The extremes are intuitive:
$\delta=0$ yields the set of maximizers, while $\delta\uparrow 1$ includes all feasible experts. The resulting
...
By construction, every expert $k\in\mathcal{S}_{t,h}(\delta)$ satisfies
\(\rho_{t,h}(k)\ge (1-\delta)\rho^{\max}_{t,h}\); the set therefore contains only near-maximal candidates.
The total mass \(\sum_{k\in\mathcal{S}_{t,h}(\delta)} \rho_{t,h}(k)\) is not forced to exceed any fixed
threshold and can be small when demand is diffuse; the rule is intentionally conservative but never
empty when a feasible expert exists.
Moreover, $\mathcal{S}_{t,h}(\delta)\subseteq \mathcal{E}^{\mathrm{feas}}_{t+h\mid t}$ for each $h$,
so each scheduled set is feasible and contained in $\mathcal{K}_t$.
...
\(\widehat{\mathcal{S}}_{t,h}(\delta)=\{k:\hat{\rho}_{t,h}(k)\ge (1-\delta)\hat{\rho}^{\max}_{t,h}\}\). The
resulting plug-in sets are denoted $\widehat{\mathcal{S}}_{t,h}(\delta)$, and the output schedule is
\(
...
so $\widehat{\mathcal{S}}_{t,h}(\delta)$ converges to $\mathcal{S}_{t,h}(\delta)$ except at ties and
...
\paragraph{Regressive planning baseline.}
For comparison, we also report a \emph{regressive} planning baseline that updates the context
trajectory using the chosen expert's forecast at each lookahead step. This couples the forecasted
contexts to the planned expert sequence and deviates from the exogenous-context assumption used for
the predictive scheduling construction above. We treat it purely as a baseline, not as part of the
formal scheduling rule.

\begin{algorithm}[H]
\caption{Predictive Scheduling via Monte Carlo}
\label{alg:predictive_schedule}
\begin{algorithmic}[1]
\STATE {\bfseries Input:} horizon $H$; scenarios $N$; tolerance $\delta$; registry $\mathcal{K}_t$; feasibility sets $(\mathcal{A}_{t+h})_{h=1}^{H}$; forecast generator $\mathcal{G}$; time-$t$ SLDS belief.
\STATE Sample scenarios $\mathbf{X}^{(n)}_{t+1:t+H}$ via \eqref{eq:mc_contexts} for $n=1,\dots,N$.
\FOR{$n=1$ to $N$}
    \FOR{$h=1$ to $H$}
        \STATE $\mathcal{E}^{\mathrm{feas}}_{t+h\mid t} \leftarrow \mathcal{K}_t \cap \mathcal{A}_{t+h}$
        \IF{$\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}=\varnothing$}
            \STATE $\widehat{I}^{(n)}_{t+h} \leftarrow \varnothing$
        \ELSE
...
\FOR{$h=1$ to $H$}
    \IF{$\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}=\varnothing$}
        \STATE $\widehat{\mathcal{S}}_{t,h}(\delta) \leftarrow \varnothing$
    \ELSE
        \STATE $\hat{\rho}^{\max}_{t,h} \leftarrow \max_{k\in\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}}\hat{\rho}_{t,h}(k)$
        \STATE $\widehat{\mathcal{S}}_{t,h}(\delta) \leftarrow \{k\in\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}:\hat{\rho}_{t,h}(k)\ge (1-\delta)\hat{\rho}^{\max}_{t,h}\}$
    \ENDIF
\ENDFOR
