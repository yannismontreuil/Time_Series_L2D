...
The central bottleneck is censoring: at round \(t\) we observe only the queried residual
\(e_t\coloneqq e_{t,I_t}\), while \((e_{t,k})_{k\neq I_t}\) remain unknown. We
...
A discrete regime \(z_t\in\{1,\dots,M\}\) selects the active dynamical trend (e.g., ``bull'' vs.\ ``crisis'').
...
\(\mathbf{u}_{0,k}\); inference only requires these to be specified and known. We give the full model implementation
details in Appendix~\ref{alg:router_main}.
