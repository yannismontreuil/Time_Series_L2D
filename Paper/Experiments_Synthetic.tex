...
The environment is fully specified by the deterministic regime schedule
\eqref{eq:synthetic_regime}, the AR(1) target and context process
\eqref{eq:synthetic_y}--\eqref{eq:synthetic_x}, the regime-conditioned shared and idiosyncratic
latent dynamics \eqref{eq:synthetic_g}--\eqref{eq:synthetic_u}, and the expert generation rule
\eqref{eq:synthetic_residual}--\eqref{eq:synthetic_prediction}. Concretely, the four experts are
parameterized by (i) regime-dependent bias means \(b_{m,k}\) (controlling which experts are best in
each regime) and (ii) shared-factor loadings \(B_k\) (controlling which experts are correlated). In
\texttt{exp\_synthetic\_1.yaml} we set \(B_0=B_1=[1,0]\) and \(B_2=B_3=[0,1]\); the shared-factor
covariances swap their dominant component across regimes,
\(Q_g^{(0)}=\mathrm{diag}(0.2,0.01)\) and \(Q_g^{(1)}=\mathrm{diag}(0.01,0.2)\), yielding
regime-dependent correlation strength for the pairs \((0,1)\) and \((2,3)\). We set bias means
\(b_{0,\cdot}=(0.2,0.3,1.6,1.7)\) and \(b_{1,\cdot}=(1.6,1.7,0.8,0.9)\), so experts \(\{0,1\}\) are
best in regime \(0\) and experts \(\{2,3\}\) are best in regime \(1\). Under \eqref{eq:synthetic_residual}
and conditional independence of idiosyncratic noise across experts, cross-expert coupling is entirely
through \(\mathbf{g}_t\); in particular,
\[
\mathrm{Cov}(e_{t,i},e_{t,j}\mid z_t=m,x_t)=x_t^2\,B_i Q_g^{(m)} B_j^\top,
\]
so the pair sharing the dominant component of \(Q_g^{(m)}\) exhibits the strongest correlation in
regime \(m\).
...
We generate each expert's residual (in the main-text convention \(e_{t,k}=\hat y_{t,k}-y_t\)) as a
linear-Gaussian function of the shared and idiosyncratic latent states:
\begin{equation}
...
\end{equation}
The expert performance is controlled by the regime-specific mean \(b_{z_t,k}\) of \(u_{t,k}\) and the
shared correlation structure \(B_k g_t\).
In particular, conditional on \((z_t=m,x_t)\) we have \(\mathbb{E}[e_{t,k}\mid z_t=m,x_t]= -x_t\,b_{m,k}\),
while cross-expert dependence arises through \(\mathbf{g}_t\) as in the covariance identity above.
...
\end{equation}
with regime-conditioned linear dynamics for \(g_t\) and \(u_{t,k}\).
The synthetic environment implements the same linear-Gaussian family up to a sign convention: our
generator uses \eqref{eq:synthetic_residual}, which differs from \eqref{eq:model_residual} by a
global sign that can be absorbed by replacing \((B_k,u_{t,k})\) with \((-B_k,-u_{t,k})\).
Thus the model family contains the true data-generating process. All regime-conditioned dynamics
match \eqref{eq:synthetic_g}--\eqref{eq:synthetic_u} with \(\phi(x_t)=x_t\) and \(d_g=2\).
