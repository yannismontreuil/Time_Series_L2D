...
\emph{bandit feedback on expert predictions}: at each round, the router observes the target \(y_t\)
but only the queried expert's prediction \(\widehat{y}_{t,I_t}\), so it can compute the signed
residual and cost for the selected expert only. Let
...
\begin{enumerate}
    ...
    the environment then reveals the target \(y_t \in \mathcal{Y}\). The observation is
    \(O_t\coloneqq (I_t,\widehat{y}_{t,I_t},y_t)\), which lets the router compute the signed residual
    \(e_t=y_t-\widehat{y}_{t,I_t}\) and cost for the queried expert.
\end{enumerate}
...
\label{eq:realized_residual}
    e_{t,k} \coloneqq y_t-\widehat{y}_{t,k} ,
\end{equation}
...
\emph{potential residuals} \(e_{t,k}=y_t-\widehat{y}_{t,k}\) from Section~\ref{sec:preliminaries}
...
k_t^{\mathrm{pred}} \in \arg\min_{k\in\mathcal{E}_t} \mathbb{E}\!\left[C_{t,k}^{\mathrm{pred}} \mid \mathcal{F}_t\right].
\end{equation}
...
\Delta_t(k)
\coloneqq
\mathbb{E}\!\left[ C_{t,k}^{\mathrm{pred}} - C_{t, k_t^{\mathrm{pred}}}^{\mathrm{pred}} \,\middle|\, \mathcal{F}_t \right]
\ge 0,
\end{equation}
where nonnegativity follows from the definition of \(k_t^{\mathrm{pred}}\).
...
In implementation, we clamp estimated \(\mathrm{IG}_t(k)\) below by a small \(\epsilon_{\mathrm{IG}}>0\)
...
\qquad h=1,\dots,H.
\end{equation}
Experts outside $\mathcal{K}^{\text{work}}_t$ are treated as infeasible even if they are available,
...
In our experiments, $(\mathcal{A}_{t+h})_{h=1}^H$ is deterministic and known at time $t$ (e.g., fixed
...
$\bigl(\hat{\rho}_{t,h}(k)\bigr)_{k\in\mathcal{K}^{\text{work}}_t}$ is a pmf supported on
