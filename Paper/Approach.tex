...
\end{equation}
and set $\mathcal{S}_{t,h}(\delta) \coloneqq \{ k_{(1)}, \dots, k_{(K_h)} \}$. If
\(\sum_{k}\rho_{t,h}(k)=0\) (i.e., \(\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}=\varnothing\)), we define
\(K_h=0\) and \(\mathcal{S}_{t,h}(\delta)=\varnothing\). The resulting
...
By construction, if \(\mathcal{E}^{\mathrm{feas}}_{t+h\mid t}\neq\varnothing\) then
\(\sum_{k\in\mathcal{S}_{t,h}(\delta)} \rho_{t,h}(k)\ge 1-\delta\), so
\[
...
and the coverage constraint is infeasible.
...
\FOR{$h=1$ to $H$}
    \IF{$\sum_{k\in\mathcal{K}^{\text{work}}_t}\hat{\rho}_{t,h}(k)=0$}
